...
router.get("/rotate/ec", async (req, res) => {
	...
	fs.writeFileSync(
		"/etc/secrets/public_keys_ec.json",
		JSON.stringify(publicJwk, null, "  ")
	);
	fs.writeFileSync(
		"/etc/secrets/private_keys_ec.json",
...
router.get("/jwks/ec", async (req, res) => {
	const publicJwk = fs.readFileSync("/etc/secrets/public_keys_ec.json");
...
router.get("/token/ec", async (req, res) => {
	 ...
	 */
	const alg = "RS256";
	const privateJwk = fs.readFileSync("/etc/secrets/private_keys_ec.json");
